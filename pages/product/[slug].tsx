...
function ProductDetails({ product }: ProductDetailsProps) {
  ...
  const mediaImages = (product.medias || []).filter(
    (media) => media.mediaUrl && media.type === PRODUCT_MEDIA_TYPE.IMAGE
  );
  ...
  const pdfFiles = (product.medias || []).filter(
    (media) => media.mediaUrl && media.type === PRODUCT_MEDIA_TYPE.PDF
  );
...
export default function ProductPage({ initialData }: ProductPageProps) {
  ...
  const handleNext = () => {
    const currentIndex = initialData?.categoryProducts?.findIndex(
      (product) => product.id === initialData?.productDetail?.id
    );
    if (currentIndex !== undefined) {
      const nextIndex =
        (currentIndex + 1) % initialData?.categoryProducts?.length;
      router.push(`/product/${initialData?.categoryProducts[nextIndex].id}`);
    }
  };
  ...
  const handlePrevious = () => {
    ...
    if (currentIndex !== undefined) {
      const previousIndex =
        (currentIndex - 1 + initialData?.categoryProducts?.length) %
        initialData?.categoryProducts?.length;
      router.push(
        `/product/${initialData?.categoryProducts[previousIndex].id}`
      );
    }
  };
                  ...
                  <div className="category-slider">
                    ...
                    >
                      {initialData?.categoryProducts
                        .filter(
                          (product) =>
                            product.id !== initialData?.productDetail.id
                        )
                        ?.map((product) => (
    ...
    return {
      props: {
        initialData: {
          ...
          categoryProducts: response.data.categoryProducts || [],
        },
      },
      revalidate: 60,
    };
